...
function Feed({ postText, setPostText, postImage, setPostImage, publishPost, posts, toggleLike, currentUser, onPostDeleted }) {
    return(
    <section className="feed-column" id="feed">
        ...
        <div className="feed-list">{posts.map((post) => <PostCard key={post.id} post={post} onLike={toggleLike} currentUser={currentUser} onDeleted={onPostDeleted} />)}</div>
    </section>
    )
}
